...
function [inv_G_Y] = inv_G_6thorder(Y, T, N_t, L, N_x, epsilon_0)
     ...
     inv_M_Y(:, :, 1) = inv_M_U(ht, L, N_x, Y(:, :, 1), epsilon_0);
     for i = 2:N_t
         inv_M_Y(:, :, i) = inv_M_U(ht, L, N_x, inv_M_Y(:, :, i-1) + Y(:, :, i), epsilon_0); 
     end
     
     inv_G_Y = zeros(N_x, N_x, N_t);
     inv_G_Y(:, :, N_t) = inv_M_U(ht, L, N_x, inv_M_Y(:, :, N_t), epsilon_0);
     for i = (N_t-1):-1:1
         inv_G_Y(:,:, i) = inv_M_U(ht, L, N_x, inv_G_Y(:, :, i+1) + inv_M_Y(:, :, i), epsilon_0);
     end
